...
class AppStore extends Component {
  ...
  filteredListApp = () => {
    const {category, searchedValue} = this.state
    const filteredList = appsList.filter(
      eachApp =>
        eachApp.category === category &&
        eachApp.appName.toLowerCase().includes(searchedValue.toLowerCase()),
    )
    return filteredList
  }

  render() {
    const filteredList = this.filteredListApp()
    const {category, searchedValue} = this.state
    return (
